...
export async function getAvailableVehicles(filters = {}) {
  ...
  if (filters.pickupBranchOfficeId != null)
    params.append("PickupBranchOfficeId", filters.pickupBranchOfficeId);
  if (filters.dropOffBranchOfficeId != null)
    params.append("DropOffBranchOfficeId", filters.dropOffBranchOfficeId);
  ...
  if (filters.transmissionType)
    params.append("TransmissionType", filters.transmissionType);
...
export async function createReservation(reservationData) {
  ...
  if (!res.ok) {
    if (res.status === 401) {
      throw new Error(
        "Los microservicios necesitan compartir la misma configuración JWT. Verifica que ambos servicios (Auth:7052 y Reservations:7055) usen la misma clave secreta JWT."
      );
    }
    try {
...
export async function cancelReservation(id) {
  const res = await fetch(RESERVATION_URLS.CANCEL_RESERVATION(id), {
    method: "POST",
    headers: getAuthHeaders(),
  });
  if (!res.ok) {
  ...
  if (!res.ok) {
    if (res.status === 409) {
      try {
        const error = await res.json();
        throw new Error(
          error.message || "La hora de retiro proporcionada no es válida."
        );
      } catch (parseError) {
        throw new Error(
          "Conflicto: La hora de retiro proporcionada no es válida. Verifica que estés dentro del horario permitido para recoger el vehículo."
        );
      }
    }
    if (res.status === 403) {
    ...
    try {
      const error = await res.json();
      throw new Error(
        error.message || "Error al registrar la retirada del vehículo"
      );
    } catch (parseError) {
      throw new Error(`Error ${res.status}: ${res.statusText}`);
    }
...
export async function returnReservation(id) {
  const res = await fetch(RESERVATION_URLS.RETURN_RESERVATION(id), {
    method: "POST",
    headers: getAuthHeaders(),
  });
  if (!res.ok) {
    const error = await res.json();
    throw new Error(
      error.message || "Error al registrar la devolución del vehículo"
    );
  }
  return await res.json();
}
...
export async function getReservationSummaryForPayment(id) {
  ...
  if (!res.ok) {
    ...
    if (res.status === 403) {
      throw new Error(
        "No tienes permisos para ver los datos de pago de esta reserva."
      );
    }
    if (res.status === 401) {
      throw new Error(
        "Error de autenticación: El microservicio de Payment no puede acceder al servicio de Reservations. Verifica que ambos servicios compartan la misma configuración JWT."
      );
    }
    if (res.status === 500) {
      throw new Error(
        "Error interno del servidor de pagos. Verifica que el microservicio de Payment esté configurado correctamente y tenga acceso al servicio de Reservations."
      );
    }
    try {
      const error = await res.json();
      throw new Error(
        error.message || "Error al obtener los datos de pago de la reserva"
      );
    } catch (parseError) {
      throw new Error(`Error ${res.status}: ${res.statusText}`);
    }
